...
func (svc *GreetService) handTargetSendMess(target apiclient.TaskTarget, config apiclient.TenantHelloConfigData) {
	...
	if err != nil {
		global.LOGGER.Error(fmt.Sprintf("greet->tenantId:%s -> SeatId类型转换失败,target: %s, err: %v", tenantId, toJSONString(target), err))
		return
	}
	...
	global.LOGGER.Info(fmt.Sprintf("greet->tenantId:%s -> 账号分配成功,target: %s, accInfo: %s", tenantId, toJSONString(target), toJSONString(accInfo)))

	// 使用可用账号发送消息
	err = svc.processGreetMessage(tenantId, target, accInfo, config)
	if err == nil {
		global.LOGGER.Info(fmt.Sprintf("greet->tenantId:%s -> seatId:%s -> 目标账号:%d -> 消息发送成功", tenantId, target.SeatId, target.TargetAcc))
	} else {
		global.LOGGER.Error(fmt.Sprintf("greet->tenantId:%s -> seatId:%s -> 目标账号:%d -> 发送消息失败: %v", tenantId, target.SeatId, target.TargetAcc, err))
	}
}
...
//			global.LOGGER.Info(fmt.Sprintf("租户id：%s ===> seatId：%s ===> 目标 %s：发送消息超时，已超过 10 分钟，抛弃", tenant.ID, target.BelongToSubUserID, target.Phone))
//			break
//		}
//		// 如果账号为空，重新拉取账号
//		if account == nil {
...
//			if account == nil {
//				global.LOGGER.Info(fmt.Sprintf("租户id：%s ===> seatId：%s ===> 目标 %s：没有可用账号，20 秒后重试", tenant.ID, target.BelongToSubUserID, target.Phone))
//				time.Sleep(20 * time.Second)
//				continue
//			}
//			global.LOGGER.Info(fmt.Sprintf("租户id：%s ===> seatId：%s ===> 目标 %s,拉取到新ws账号: %s。", tenant.ID, target.BelongToSubUserID, target.Phone, account.Username))
//		}
//		// 使用可用账号发送消息
...
//		if err != nil {
//			errMsg := fmt.Sprintf("seatId转换错误:%s", err)
//			global.LOGGER.Error(errMsg)
//			return err
//		}
//
...
//	}
//
// processGreetMessage 处理问候消息发送
func (svc *GreetService) processGreetMessage(tenantId string, target apiclient.TaskTarget, accInfo *apiclient.AccountData, config apiclient.TenantHelloConfigData) error {
	// TODO: 实现具体的消息发送逻辑
	// 这里应该根据config.Type来处理不同类型的消息（text/link）
	// 并使用accInfo中的账号信息来发送消息到target.TargetAcc

	global.LOGGER.Info(fmt.Sprintf("greet->tenantId:%s -> 开始处理消息发送, target: %s, accInfo: %+v, config: %+v",
		tenantId, toJSONString(target), accInfo, toJSONString(config)))

	// 临时返回nil表示成功，实际应该实现具体的发送逻辑
	return nil
}
